...
export function BlueprintViewer({ blueprint, locale = 'en', userName, userEmail }: BlueprintViewerProps) {
  ...
  const handleExportPDF = async () => {
    try {
      ...
      const headerHeight = 15; // Space for header with metadata
      const footerHeight = 10; // Space for footer
      const contentPageHeight = pageHeight - headerHeight - footerHeight;
      ...
      const exportTimestamp = new Date().toLocaleString(locale === 'bn' ? 'bn-BD' : 'en-US', {
        year: 'numeric',
        month: 'long',
        day: 'numeric',
        hour: '2-digit',
      ...
      const exportUser = userName || userEmail || 'Unknown User';

      // Add header with timestamp and user info on first page
      pdf.setFontSize(8);
      pdf.setTextColor(100, 100, 100);
      pdf.text(`Exported by: ${exportUser}`, 10, 8);
      pdf.text(`Export date: ${exportTimestamp}`, 10, 12);
      pdf.text(`Blueprint: ${blueprint?.productIdea || 'Business Blueprint'}`, 120, 8);
      pdf.text(`Type: ${blueprint?.businessType || 'N/A'}`, 120, 12);
      ...
      while (heightLeft >= 0) {
        ...
        pdf.setFontSize(8);
        pdf.setTextColor(100, 100, 100);
        pdf.text(`Exported by: ${exportUser} | ${exportTimestamp}`, 10, 8);
        pdf.addImage(imgData, 'PNG', 0, position, imgWidth, imgHeight);
        heightLeft -= contentPageHeight;
      }
      ...
      for (let i = 1; i <= pageCount; i++) {
        ...
        pdf.text(
          `Page ${i} of ${pageCount} | Generated by VentureOS`,
          105,
          pageHeight - 5,
          { align: 'center' }
        );
